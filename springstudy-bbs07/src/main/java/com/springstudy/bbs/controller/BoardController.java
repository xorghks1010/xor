...
public class BoardController {
	...
	// 게시 글쓰기(다중 파일 업로드) 폼에서 들어오는 게시 글쓰기 요청을 처리하는 메서드
	...
	/* BadRequest(400) Exception 처리 메서드
	 * 이 Controller에서 TypeMismatchException이 발생하면 이 메서드가 
	 * 실행된다. TypeMismatchException은 요청 파라미터가 Controller 메서드의
	 * 파라미터로 변환할 수 없는 타입일 때 발생한다.
	 **/
